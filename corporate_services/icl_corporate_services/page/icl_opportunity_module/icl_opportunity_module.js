frappe.pages["icl-opportunity-module"].on_page_load = function (wrapper) {
  ...
  $wrapper
    .empty()
    .append('<div id="opportunity-module-root" class="opportunity-module p-0"></div>');

  // Prepare Frappe's right sidebar host for the React module.
...
frappe.pages["icl-opportunity-module"].on_page_show = function () {
  ...
  // route[0] = 'icl-opportunity-module', route[1] = selected item id (optional)
  const selectedId = route[1] || null;
  if (globalThis.opportunityModuleSetRoute) {
    globalThis.opportunityModuleSetRoute(selectedId);
  }
};
